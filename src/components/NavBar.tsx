...
export function NavBar({ currentView, setView }: { currentView: string, setView: (v: 'list' | 'detail' | 'discovery' | 'flights' | 'subscriptions') => void }) {
  return (
    <nav className="sticky top-0 z-40 w-full bg-paper/90 backdrop-blur-md border-b border-graphite/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6 text-xs uppercase tracking-widest font-medium text-graphite/60">
          ...
          <button onClick={() => setView('subscriptions')} className={`hover:text-graphite transition-colors ${currentView === 'subscriptions' ? 'text-graphite' : ''}`}>Subscriptions</button>
